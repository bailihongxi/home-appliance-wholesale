...
test('关于：版本号与 PRD / 开发计划一致（V3.69）', () => {
  const { ctx, state } = fresh();
  const html = page.render(ctx, state);
  assert.ok(html.includes('版本：V3.83'), '关于页显示 V3.69');
  ...
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.27');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.26');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.23');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.22');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.16');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.15');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.14');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.9');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.8');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.7');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.6');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.3');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示旧版本号 V3.1');
  assert.ok(!html.includes('版本：V3.80）'), '不再显示更旧版本号');
});
...
test('V3.4-同步接线：syncUp/syncDown 传入脱敏账户档案，恢复后写回账户设置（店铺名/头像/经营范围，不含密码）', () => {
  ...
  // 上传与恢复都携带账户公开档案（V3.62：改用 syncAccountPublic——共用本店数据时写归属账号（老板）档案，
  // 避免员工把自己的店名/头像写进全店共享快照；独立数据空间仍等价于当前账号档案）
  assert.ok(src.includes('sync.syncUp(ctx, state.cfg, undefined, syncAccountPublic())'), '同步到云端携带账户档案');
  assert.ok(src.includes('sync.syncDown(ctx, state.cfg, undefined, syncAccountPublic())'), '从云端恢复携带账户档案');
