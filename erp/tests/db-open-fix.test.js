...
test('db open：新库创建全部 store', async () => {
  ...
  const d = await db.create({ backend: db.indexedDbBackend(fake), name: 'erp_new' });
  ALL_STORES.forEach((s) => assert.ok(d.stores.includes(s), s + ' 应存在'));
});

test('db open：已存在但缺 store 的残留库 → 自动补齐', async () => {
  // 模拟：erp_acct2 库已存在（version 1）但没有任何 store（坏库）
  const fake = makeFakeIndexedDB([{ name: 'erp_acct2', version: 1, stores: [] }]);
  const d = await db.create({ backend: db.indexedDbBackend(fake), name: 'erp_acct2' });
  ALL_STORES.forEach((s) => {
    assert.ok(d.stores.includes(s), '补齐后 ' + s + ' 应存在');
  });
  const rec = fake._dbs.get('erp_acct2');
...
test('db open：部分缺失 store 也补齐且不破坏已有 store', async () => {
  ...
  const fake = makeFakeIndexedDB([{ name: 'erp_part', version: 1, stores: ['products', 'meta'] }]);
  const d = await db.create({ backend: db.indexedDbBackend(fake), name: 'erp_part' });
  ALL_STORES.forEach((s) => {
    assert.ok(d.stores.includes(s), '部分补齐后 ' + s + ' 应存在');
  });
  const rec = fake._dbs.get('erp_part');
  assert.ok(rec.stores.has('products'), '已有 store 保留');
  assert.ok(rec.stores.has('meta'));
});

test('db open：完整库不触发无谓升级', async () => {
  const fake = makeFakeIndexedDB([{ name: 'erp_ok', version: 1, stores: ALL_STORES }]);
  const d = await db.create({ backend: db.indexedDbBackend(fake), name: 'erp_ok' });
  assert.strictEqual(fake._dbs.get('erp_ok').version, 1, '完整库不升级版本');
...
test('db open：已升级到高版本(2)的完整库可再次打开，不抛版本过低错误', async () => {
  ...
  const fake = makeFakeIndexedDB([{ name: 'erp_up', version: 2, stores: ALL_STORES }]);
  const d = await db.create({ backend: db.indexedDbBackend(fake), name: 'erp_up' });
  assert.strictEqual(fake._dbs.get('erp_up').version, 2, '不降级不升级，保持 version 2');
