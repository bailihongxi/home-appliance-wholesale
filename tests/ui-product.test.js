...
test('全选与当前页渲染一致：乱序插入 250 款，勾选集合=显示集合（V3.58：页大小 100）', () => {
  const { ctx, state } = fresh();
  ...
  assert.strictEqual(rowIds.length, 100, '第 1 页 100 行（V3.58 统一）');
  assert.ok(rowIds.every(id => state.sel[id]), '当前页每一行都在选中集合（行勾选标志可见）');
  assert.ok(html.includes('删除选中（100）'), '删除按钮计数 100');
  const checkedRows = (html.match(/class="row-check" data-act="row-check" data-id="[^"]*" checked/g) || []).length;
  assert.strictEqual(checkedRows, 100, '100 行全部带 checked 勾选标志');
  // 勾选集合不多不少：恰为当前页 100 个 id
  assert.strictEqual(Object.keys(state.sel).filter(k => state.sel[k]).length, 100);
});
